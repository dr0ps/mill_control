        ...
        match HidApi::new() {
            Ok(api) => {
                for device in api.device_list() {
                    println!("{:04x}:{:04x}, {}", device.vendor_id(), device.product_id(), device.manufacturer_string().or(Some("None")).unwrap());
                            ...
                            Ok(device) => {
                                loop {
                                    ...
                                    if device.read_timeout(&mut input_buf, -1).is_ok() {
                                        ...
                                        let mut output_buf: [u8; 8] = [0x06, 0xFE, 0xFD, 0xFF, 0x00, pos_x[1], pos_x[0], pos_x[3]];
                                        device.send_feature_report(&output_buf);
                                        let mut output_buf: [u8; 8] = [0x06, pos_x[2], pos_y[1], pos_y[0], pos_y[3], pos_y[2], pos_z[1], pos_z[0]];
                                        device.send_feature_report(&output_buf);
                                        let mut output_buf: [u8; 8] = [0x06, pos_z[3], pos_z[2], 0x00, 0x00, 0x00, 0x00, 0x00];
                                        device.send_feature_report(&output_buf);
                                    }
                                }
                            }
